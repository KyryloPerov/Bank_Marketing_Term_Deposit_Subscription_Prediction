...
```
.
├── Bank_Marketing_Term_Deposit_Prediction.ipynb   # Main notebook (all analysis)
...
| `month` | One-hot encoding | Kept as a plain categorical — **no** target-derived season grouping (avoids leakage; see below). Trees recover the seasonal signal on their own |
| `education` → `education_ord` | Ordinal encoding (0–6) | Natural ordinal order exists; `unknown` mapped to median level 3 |
| `campaign` | Cap at 99th percentile (**train-only**) | Long right tail (max=56); cap threshold learned on the train split inside `prepare_train_val`, applied to both sets |
...
An earlier version grouped `month` into `high/mid/low_season` tiers by conversion
rate. Those tiers were derived from the target over the **whole dataset**, while
`engineer_features()` runs **before** the train/val split — so the grouping encoded
validation-row labels into the features (mild but real target leakage). The fix:
drop the season feature and one-hot encode `month` directly. On the same principle,
every fit-on-data step (`campaign` cap, `StandardScaler`) is now learned on the
**training split only**, inside `prepare_train_val`.
...
| LightGBM (RandomizedSearchCV) | 50 iterations, 5-fold CV | — | ~0.81 | — | Improved over base |
| LightGBM (Hyperopt TPE) | 50 evaluations, 5-fold CV | — | **~0.81+** | — | **Best overall** |

> Figures are from a reference run. After the leakage fix (dropping the
> target-derived season feature), re-running reproduces very similar values —
> the exact numbers are generated by the notebook cells.
...
jupyter notebook Bank_Marketing_Term_Deposit_Prediction.ipynb
```
...
- A LightGBM model tuned with Hyperopt achieves **Val AUC ≈ 0.80–0.81**, a large improvement over the random baseline.
- Feature engineering (`age_group`, `contacted_before`) and clean, leakage-free preprocessing contributed meaningfully to model quality.
